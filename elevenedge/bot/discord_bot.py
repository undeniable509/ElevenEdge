...
async def upload(interaction: discord.Interaction, video: discord.Attachment) -> None:
    ...
    if response.ok:
        ...
        await interaction.followup.send(
            '✅ Upload accepted. '
            f"video_id={payload['video_id']} status={payload['status']}. "
            'Run /search after the worker finishes transcription.'
        )
    else:
...
async def search(interaction: discord.Interaction, video_id: int, query: str) -> None:
    ...
    if not matches:
        await interaction.followup.send(
            'ℹ️ No matches found yet. The transcript may still be processing, or the query had no hits.'
        )
        return

    message = '\n'.join([f"{m['timestamp']} - {m['text']}" for m in matches])
    await interaction.followup.send(f'✅ Top matches:\n{message}')


@bot.tree.command(name='clip', description='Generate a clip from transcript timestamps')
@app_commands.describe(video_id='Video ID', start='Segment start (seconds)', end='Segment end (seconds)')
async def clip(interaction: discord.Interaction, video_id: int, start: float, end: float) -> None:
    ...
    await interaction.followup.send(
        f"✅ Clip ready. clip_id={payload['clip_id']} start={payload['start_time']:.2f}s end={payload['end_time']:.2f}s"
    )
